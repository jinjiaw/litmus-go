...
import (
	...
	experimentTypes "github.com/litmuschaos/litmus-go/pkg/sample-category/node-cordon/types"
...
//PrepareNodeCordon contains the prepration steps before chaos injection
func PrepareNodeCordon(experimentsDetails *experimentTypes.ExperimentDetails, clients clients.ClientSets, resultDetails *types.ResultDetails, eventsDetails *types.EventDetails, chaosDetails *types.ChaosDetails) error {
	...
	if err := cordonNode(experimentsDetails, clients, chaosDetails); err != nil {
		return err
	}
...
// cordonNode cordon the application node
func cordonNode(experimentsDetails *experimentTypes.ExperimentDetails, clients clients.ClientSets, chaosDetails *types.ChaosDetails) error {
	...
	case <-inject:
		...
		os.Exit(0)
	default:
		log.Infof("[Inject]: Cordoning the %v node", experimentsDetails.TargetNode)

		command := exec.Command("kubectl", "cordon", experimentsDetails.TargetNode)
		var out, stderr bytes.Buffer
		command.Stdout = &out
		command.Stderr = &stderr
		if err := command.Run(); err != nil {
			log.Infof("Error String: %v", stderr.String())
			return errors.Errorf("Unable to cordon the %v node, err: %v", experimentsDetails.TargetNode, err)
		}
